...
create table if not exists dead_token_universe (
  ...
  price_change_json text,  -- GMGN momentum: {1h, 6h, 24h} percent, backfill per scan
...
-- alter table dead_token_universe add column if not exists price_change_json text;
